...
CREATE TABLE IF NOT EXISTS stripe_payments (
  ...
  program_purchase_id uuid REFERENCES program_purchases(id) ON DELETE SET NULL,
  metadata jsonb DEFAULT '{}'::jsonb,
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
);
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE professor_subscriptions ADD COLUMN price_paid numeric(10, 2) DEFAULT 0;
  END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_stripe_payments_program_purchase_id ON stripe_payments(program_purchase_id);
